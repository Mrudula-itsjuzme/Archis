...
const NavItem = ({ icon, label, active = false, onClick }: { icon: React.ReactNode; label: string; active?: boolean; onClick?: () => void }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-3 px-3 py-2.5 w-full rounded-lg text-sm font-medium transition-all ${active ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}
  >
...
export default function Sidebar() {
  ...
  const handleNewProject = () => {
    useStore.getState().resetModel();
    useStore.getState().setBlueprintConfig({ url: null });
  ...
  return (
    <div className="w-56 h-full bg-white border-r border-gray-200 flex flex-col shrink-0 overflow-hidden">
      ...
      <div className="flex flex-col gap-0.5 p-3">
        <NavItem active={activeNav === 'home'} label="Home" onClick={() => { setActiveNav('home'); useStore.getState().setWorkspaceMode('plan'); }} icon={
        ...
        <NavItem label="New Project" onClick={handleNewProject} icon={
        ...
        <NavItem label="Projects" onClick={() => setActiveNav('projects')} icon={
        ...
        <NavItem label="Settings" onClick={() => setActiveNav('settings')} icon={
            ...
            <div className="flex flex-col gap-2">
              {useStore.getState().savedProjects.map((p: any) => (
                <button
                  key={p.id}
                  onClick={() => {
                    useStore.getState().loadSavedProject(p.id);
                    setActiveNav('blueprint');
                  }}
